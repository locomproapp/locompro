...
const CompactOfferOwnerActions = ({ 
  ...
  // Don't show any actions for finalized offers
  if (status === 'finalized') {
    return null;
  }
      ...
      {(status === 'pending' || status === 'rejected') && (
        <>
          <Button
            ...
            size="sm"
            asChild
            className={isMobile ? "flex-[2] text-xs min-h-[36px] h-9 px-2 min-w-0 max-w-none flex items-center justify-center" : "flex-1 text-xs md:text-xs min-h-[36px] h-9 px-2 md:px-3 min-w-0 max-w-none md:min-w-[100px] md:max-w-[140px] flex items-center justify-center"}
          >
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button
                ...
                size="sm"
                disabled={isDeleting}
                className={isMobile ? "w-9 text-xs min-h-[36px] h-9 px-0 min-w-0 max-w-none text-destructive hover:text-destructive hover:border-destructive flex items-center justify-center" : "flex-1 text-xs md:text-xs min-h-[36px] h-9 px-2 md:px-3 min-w-0 max-w-none md:min-w-[100px] md:max-w-[140px] text-destructive hover:text-destructive hover:border-destructive flex items-center justify-center"}
              >
                <Trash2 className="h-3 w-3 flex-shrink-0" />
                {!isMobile && (
                  <span className="whitespace-nowrap ml-1 md:ml-1.5">
